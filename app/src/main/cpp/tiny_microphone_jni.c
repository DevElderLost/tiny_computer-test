...
typedef struct {
    ...
    /* ---- realtime monitor (mic -> speaker loopback) ---- */
    atomic_bool    monitor_enabled;
    AAudioStream  *monitor_stream;
} state_t;
 ...
 */
/* ================================================================= */
/*  Realtime monitor (mic -> speaker), opened lazily, lives for the  */
/*  whole capture session -- toggling on/off just flips a flag so    */
/*  there is no open/close race with the realtime data_callback.     */
/* ================================================================= */
static int monitor_open(int32_t rate, int32_t channels)
{
    ...
    if (res != AAUDIO_OK) { LOGE("monitor createBuilder: %d", (int)res); return -1; }
    ...
    if (res != AAUDIO_OK) { LOGE("monitor openStream: %d", (int)res); return -1; }

    res = AAudioStream_requestStart(s);
    if (res != AAUDIO_OK) {
        LOGE("monitor requestStart: %d", (int)res);
        AAudioStream_close(s);
        return -1;
    }

    g.monitor_stream = s;
    LOGI("monitor stream opened: rate=%d ch=%d", (int)rate, (int)channels);
    return 0;
}

static void monitor_close(void)
{
    if (g.monitor_stream) {
        AAudioStream_close(g.monitor_stream);
        g.monitor_stream = NULL;
    }
}
        ...
        int32_t num_frames)
{
    ...
    /* Realtime monitor (mic -> speaker): best-effort, non-blocking write. */
    if (atomic_load_explicit(&g.monitor_enabled, memory_order_acquire) && g.monitor_stream) {
        AAudioStream_write(g.monitor_stream, audio_data, num_frames, 0);
    }
                                            ...
                                            jstring socketPath)
{
    ...
    atomic_init(&g.monitor_enabled, false);
...
atomic_store_explicit(&g.monitor_enabled, false, memory_order_release);
monitor_close();
...
JNIEXPORT void JNICALL
Java_com_fct_tc4_TinyMicrophone_nativeSetMonitorEnabled(JNIEnv *env, jclass cls, jboolean enabled)
{
(void)env; (void)cls;
...
if (enabled) {
    if (!g.monitor_stream) {
        int32_t rate = g.sample_rate > 0 ? g.sample_rate : 48000;
        int32_t ch   = g.channel_count > 0 ? g.channel_count : 1;
        if (monitor_open(rate, ch) != 0) {
            LOGE("failed to enable monitor");
            return;
        }
    }
    atomic_store_explicit(&g.monitor_enabled, true, memory_order_release);
